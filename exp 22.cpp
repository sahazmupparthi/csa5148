...
void cbcEncrypt(uint8_t *plaintext, uint8_t *ciphertext, uint8_t *key, uint8_t iv, int numBlocks);
void cbcDecrypt(uint8_t *ciphertext, uint8_t *plaintext, uint8_t *key, uint8_t iv, int numBlocks);
...
void cbcEncrypt(uint8_t *plaintext, uint8_t *ciphertext, uint8_t *key, uint8_t iv, int numBlocks) {
    uint8_t prevCiphertext = iv;
    for (int i = 0; i < numBlocks; i++) {
        plaintext[i] ^= prevCiphertext;
        sdesEncrypt(&plaintext[i], key);
...
void cbcDecrypt(uint8_t *ciphertext, uint8_t *plaintext, uint8_t *key, uint8_t iv, int numBlocks) {
    ...
    for (int i = 0; i < numBlocks; i++) {
        ...
        sdesDecrypt(&ciphertext[i], key);
        plaintext[i] = ciphertext[i] ^ prevCiphertext;
        prevCiphertext = temp;
    }
}
